...
pid=$( curl --silent --user "${publicApiKey}:${privateApiKey}" --digest \
 ...
 --request GET "${opsMgrUrl}/api/public/v1.0/groups/byName/${project}?pretty=true" |jq .id )
...
jobid=$( curl --silent --user "${publicKey}:${privateKey}" --digest \
 --header "Accept: application/json" \
 --header "Content-Type: application/json" \
 --request POST "${opsMgrUrl}/api/public/v1.0/groups/${projectId}/logCollectionJobs?pretty=true" \
 --data '
   {
...
curl --silent --user "${publicKey}:${privateKey}" --digest \
 --header "Accept: application/json" \
 --header "Content-Type: application/json" \
 --request GET "${opsMgrUrl}/api/public/v1.0/groups/${projectId}/logCollectionJobs?pretty=true" 
