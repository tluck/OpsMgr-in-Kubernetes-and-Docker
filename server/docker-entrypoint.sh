...
#chown mongodb-mms /etc/mongodb-mms
...
gosu root curl -OL http://opsmgr:8080/download/agent/automation/mongodb-mms-automation-agent-manager_10.12.2.6205-1_amd64.ubuntu1604.deb \
    && gosu root dpkg -i mongodb-mms-automation-agent-manager_10.12.2.6205-1_amd64.ubuntu1604.deb

cp /shareddata/automation-agent.config /etc/mongodb-mms/automation-agent.config
nohup /opt/mongodb-mms-automation/bin/mongodb-mms-automation-agent --config=/etc/mongodb-mms/automation-agent.config >> /var/lib/mongodb-mms-automation/automation-agent-fatal.log 2>&1 &

# start mongodb for ops manager
#mongod -f /shareddata/mongod.conf --logpath /data/db/mongodb.log --fork
exec "/bin/bash"
